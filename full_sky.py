...
from masking_healpy_kdtree_great_circle import parallel_mask_stars
gaia_cat = Table.read('/data/star_masking/Gaia_subset_dered_l_b_hpid.fits')
...
if test:
    ...
    files = glob(os.path.join(datadir, f'*s000[123456789]-[1234]-[1234]*fits'))
    files += glob(os.path.join(datadir, f'*s001[0123456789]-[1234]-[1234]*fits'))
    files += glob(os.path.join(datadir, f'*s002[0123456789]-[1234]-[1234]*fits'))
    files += glob(os.path.join(datadir, f'*s003[0123456789]-[1234]-[1234]*fits'))
    files += glob(os.path.join(datadir, f'*s004[0123456789]-[1234]-[1234]*fits'))
    files += glob(os.path.join(datadir, f'*s005[0123456789]-[1234]-[1234]*fits'))
    files += glob(os.path.join(datadir, f'*s006[01234567]-[1234]-[1234]*fits'))
...
for ii, ifile in enumerate(files):
    ...
    with fits.open(ifile) as ff:
        ...
        elif isec == 62 and icam == 3 and iccd == 2:
            data[:600, 1300:] = np.nan
        elif isec == 63 and icam == 1 and iccd == 3:
            data[:150, 100:350] = np.nan
        elif isec == 63 and icam == 1 and iccd == 4:
            data[:150, 1900:] = np.nan
            data[450:600, 1950:] = np.nan
        elif isec == 63 and icam == 2 and iccd == 1:
            data[:400, :600] = np.nan
        elif isec == 63 and icam == 2 and iccd == 2:
            data[:900, 1100:] = np.nan
        elif isec == 63 and icam == 3 and iccd == 2:
            data[:700, 1400:] = np.nan
        elif isec == 64 and icam == 1 and iccd == 1:
            data[:200, :300] = np.nan
        elif isec == 64 and icam == 2 and iccd == 1:
            data[:500, :800] = np.nan
        elif isec == 64 and icam == 3 and iccd == 1:
            data[:400, :600] = np.nan
        elif isec == 64 and icam == 3 and iccd == 2:
            data[:200, 1400:] = np.nan
        elif isec == 64 and icam == 4 and iccd == 2:
            data[:150, 1300:1600] = np.nan
        elif isec == 64 and icam == 4 and iccd == 3:
            data[:700, :500] = np.nan
        elif isec == 65 and icam == 1 and iccd == 2:
            data[:800, 1400:] = np.nan
        elif isec == 65 and icam == 1 and iccd == 3:
            data[:700, :900] = np.nan
        elif isec == 65 and icam == 2 and iccd == 1:
            data[:500, :400] = np.nan
        elif isec == 65 and icam == 2 and iccd == 2:
            data[:400, 1400:] = np.nan
        elif isec == 65 and icam == 2 and iccd == 3:
            data[:400, :700] = np.nan
            # super saturated star causing artifacts
            data[1930:2000, 1956:1974] = 1e3
            data[:1930, 1956:1974] = np.nan
        ...
        # still in (with fits.open...)
        wcs = WCS(ff[1].header)

    copy_data = data.flatten()
    valid_data = copy_data[np.isfinite(copy_data)]
    sixtp = np.nanpercentile(valid_data, 16)
    fits_stdev_l = np.nanmedian(valid_data)-sixtp
    print(fits_stdev_l)

    data[data>5e4] = np.nan
    med = np.nanmedian(copy_data)
    data[data<med-fits_stdev_l*2] = np.nan

    
    try:
        masked_data = parallel_mask_stars(data, wcs, gaia_cat)
        data = masked_data
    except Exception as e:
        print(f"Error during star masking for file {ifile}: {str(e)}")
    ...
    # galactic coords
    galactic_coords = sky_coords.transform_to('galactic')
    ...
    nside = 2048
    pixel = hp.ang2pix(nside, l_f, b_f, lonlat=True)

    unique_elements, inverse_indices = np.unique(pixel, return_inverse=True)

    # Initialize arrays for sum and count
    sum_values = np.zeros_like(unique_elements, dtype=float)
    count = np.zeros_like(unique_elements, dtype=int)

    # Use numpy's bincount to sum values and count non-NaN entries
    non_nan_mask = ~np.isnan(data_f)
    sum_values = np.bincount(inverse_indices[non_nan_mask], 
                            weights=data_f[non_nan_mask], 
                            minlength=len(unique_elements))
    count = np.bincount(inverse_indices[non_nan_mask], 
                        minlength=len(unique_elements))


    '''
    npix=hp.nside2npix(nside)
    hdata=np.full(npix,np.nan) #full with 0 might be better
    hdata[unique_elements]=sum
    '''

    # Save these arrays to a .npz file with specified keys
    
    npz_name = f'/data/tess_fullsky/tess_npz_notmasking_0820/sector{isec}-{icam}-{iccd}_-2sigma_50k_masking.npz'
    print(npz_name)

    np.savez(npz_name, pix_id=unique_elements, pix_c=count, pix_value=sum_values)
